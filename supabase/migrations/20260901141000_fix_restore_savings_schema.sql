-- Fix restore RPC for the live savings schema.
-- Production `savings` has no `user_id` column, so the restore function must
-- write only the columns that exist there.
