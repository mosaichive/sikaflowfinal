...
import {
  ...
  Tag, Users, MapPin, Check, PartyPopper, Package, Plus, X,
} from "lucide-react";
...
function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setForm((f) => ({
        ...
        products: parsed.products?.length ? parsed.products : [{ name: "", price: "", stock: "" }],
      }));
    })();
  ...
  const steps = useMemo(() => [
    ...
    { kind: "field" as const, key: "location", icon: MapPin, title: "Where are you located?", subtitle: "City, region, or country.", placeholder: "e.g. Accra, Ghana" },
    { kind: "products" as const },
    { kind: "summary" as const },
  ], []);

  const totalUserSteps = steps.length - 1;
  ...
  async function finish() {
    ...
    if (profileErr) { setSaving(false); return toast.error(profileErr.message); }
    ...
    if (validProducts.length > 0) {
      const { error: prodErr } = await supabase.from("products").insert(validProducts);
      if (prodErr) {
        setSaving(false);
        return toast.error(`Saved profile but products failed: ${prodErr.message}`);
      }
    }
  ...
  const canAdvance = (() => {
    if (current.kind === "welcome" || current.kind === "summary" || current.kind === "products") return true;
    const v = form[current.key as keyof Omit<Form, "products">];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <header className="mx-auto flex w-full max-w-3xl items-center justify-between px-6 py-5">
        <Logo />
        {step > 0 && (
          <span className="text-sm text-muted-foreground">Step {step} of {totalUserSteps}</span>
        )}
      </header>
      ...
      <main className="flex flex-1 items-center justify-center px-6 py-10">
        <div key={step} className={`w-full max-w-xl ${animClass}`}>
          <div className="rounded-3xl border border-border bg-card p-6 shadow-elegant sm:p-10">
            {current.kind === "welcome" && (
              <div className="text-center">
                ...
                <p className="mt-3 text-muted-foreground">
                  A few quick questions — under a minute. Your{" "}
                  <span className="font-medium text-foreground">30-day free trial</span> is already active.
                </p>
                <Button onClick={next} size="lg" className="mt-8 bg-primary hover:bg-primary/90">
            ...
            {current.kind === "field" && (
              <FieldStep
                ...
                value={form[current.key as keyof Omit<Form, "products">] as string}
                onChange={(v) => update(current.key as keyof Omit<Form, "products">, v as never)}
            ...
            {current.kind === "choice" && (
              <ChoiceStep
                ...
                value={form[current.key as keyof Omit<Form, "products">] as string}
                onChange={(v) => {
                  update(current.key as keyof Omit<Form, "products">, v as never);
            ...
            {current.kind !== "welcome" && (
              <div className="mt-8 flex items-center justify-between gap-3">
                ...
                {current.kind === "summary" ? (
                  ...
                  <Button onClick={next} disabled={!canAdvance} className="bg-primary hover:bg-primary/90">
  ...
  return (
    <div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="mt-6">
        ...
        <Input id="field" autoFocus placeholder={placeholder} value={value}
          onChange={(e) => onChange(e.target.value)} className="h-12 text-base" />
      </form>
    </div>
  );
}
...
function ProductsStep({ products, onChange }: { products: StarterProduct[]; onChange: (p: StarterProduct[]) => void }) {
  function update(i: number, patch: Partial<StarterProduct>) {
    onChange(products.map((p, idx) => (idx === i ? { ...p, ...patch } : p)));
  }
  function add() {
    onChange([...products, { name: "", price: "", stock: "" }]);
  }
  function remove(i: number) {
    onChange(products.length === 1 ? [{ name: "", price: "", stock: "" }] : products.filter((_, idx) => idx !== i));
  }
  ...
  return (
    <div>
      ...
      <h2 className="mt-5 text-2xl font-semibold tracking-tight">Add a few starter products</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        Optional — anything you add here will appear in your Products section instantly. You can skip this and add them later.
      </p>
      <div className="mt-5 space-y-3">
        {products.map((p, i) => (
          <div key={i} className="grid gap-2 rounded-xl border border-border bg-background/40 p-3 sm:grid-cols-[1fr_110px_110px_auto]">
            <Input placeholder="Product name" value={p.name} onChange={(e) => update(i, { name: e.target.value })} />
            <Input type="number" inputMode="decimal" min="0" step="0.01" placeholder="Price" value={p.price} onChange={(e) => update(i, { price: e.target.value })} />
            <Input type="number" inputMode="numeric" min="0" placeholder="Stock" value={p.stock} onChange={(e) => update(i, { stock: e.target.value })} />
...
function SummaryStep({ form, onEdit }: { form: Form; onEdit: (stepIndex: number) => void }) {
  const validProducts = form.products.filter((p) => p.name.trim().length > 0);
  const rows: { label: string; value: string; stepIndex: number }[] = [
    ...
    { label: "Starter products", value: validProducts.length ? `${validProducts.length} ready to add` : "None — add later", stepIndex: 7 },
  ];
  return (
    <div>
